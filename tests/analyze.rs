...
//     let mut collector = ClickHouseFunctionCollector::new(vec!["exp".to_string()],
// lineage_visitor);     let _ = plan.visit(&mut collector)?;
...
//     // Create three tables
//     drop(
//         ctx.sql("CREATE TABLE employees (id INT, name VARCHAR, salary DECIMAL, department_id
// INT)")             .await?,
//     );
//     drop(ctx.sql("CREATE TABLE departments (id INT, name VARCHAR, budget DECIMAL)").await?);
//     drop(
//         ctx.sql("CREATE TABLE bonuses (employee_id INT, bonus_amount DECIMAL, multiplier
// DECIMAL)")             .await?,
//     );
...
//     assert!(table_names.contains(&"departments".to_string()), "Should reference departments
// table");     assert!(table_names.contains(&"bonuses".to_string()), "Should reference bonuses
// table");
